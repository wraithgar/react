...
export function PickerContextProvider({children, config, recentlyUsed, onEmojiClick}) {
...
export function useCloseVariationMenu() {
  ...
  return ({target}) => {
    ...
    if (skinToneSpreadValue && target.getAttribute(PROPERTY_DATA_NAME) !== DATA_NAME) {
      collapseSkinTones()
    }
  }
}
